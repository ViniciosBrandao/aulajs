...
4 - Idoso 2450 mg
*/
function obterDosagemMg(pacienteTipo){
    const bebe = 1;
    const adolescente = 2;
    const adulto = 3;
    const idoso  = 4;

    if (pacienteTipo === bebe) return 600;
    if (pacienteTipo === adolescente) return 1600;
    if (pacienteTipo === adulto) return 4600;
    if (pacienteTipo === idoso) return 2450;
}
